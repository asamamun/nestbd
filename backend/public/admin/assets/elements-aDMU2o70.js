import{n as e}from"./rolldown-runtime-CrqDHxfU.js";import{t}from"./vendor-ui-BQHoO1Qu.js";import{t as n}from"./search-component-Cxnj3bUP.js";var r=e({elementsData:()=>i}),i=[{id:`buttons`,title:`Buttons`,category:`components`,icon:`bi bi-square`,description:`Bootstrap button styles, sizes, and states`,examples:12,url:`/elements-buttons.html`,preview:`
        ...
        `}];document.addEventListener(`alpine:init`,()=>{t.data(`elementsComponent`,()=>({components:i,filteredComponents:i,viewMode:`grid`,searchQuery:``,categoryFilter:``,init(){this.filteredComponents=this.components,console.log(`🧩 Elements component initialized`)},filterComponents(){let e=this.components;if(this.searchQuery){let t=this.searchQuery.toLowerCase();e=e.filter(e=>e.title.toLowerCase().includes(t)||e.description.toLowerCase().includes(t)||e.category.toLowerCase().includes(t))}this.categoryFilter&&(e=e.filter(e=>e.category===this.categoryFilter)),this.filteredComponents=e},toggleView(){this.viewMode=this.viewMode===`grid`?`list`:`grid`,localStorage.setItem(`elements-view-mode`,this.viewMode)},navigateToComponent(e){window.location.href=e.url},showAllComponents(){this.searchQuery=``,this.categoryFilter=``,this.filteredComponents=this.components},clearFilters(){this.searchQuery=``,this.categoryFilter=``,this.filterComponents()},getComponentCount(e){return this.components.filter(t=>t.category===e).length}})),t.data(`searchComponent`,n({getResults(e){let t=e.toLowerCase(),n=i.filter(e=>e.title.toLowerCase().includes(t)||e.description.toLowerCase().includes(t)).map(e=>({title:e.title,url:e.url,type:`element`})),r=[{title:`Dashboard`,url:`/`,type:`page`},{title:`Analytics`,url:`/analytics`,type:`page`},{title:`Users`,url:`/users`,type:`page`},{title:`Elements`,url:`/elements`,type:`page`}].filter(e=>e.title.toLowerCase().includes(t));return[...n,...r].slice(0,8)}}))}),document.addEventListener(`DOMContentLoaded`,()=>{[...document.querySelectorAll(`[data-bs-toggle="tooltip"]`)].forEach(e=>new bootstrap.Tooltip(e))});export{r as t};